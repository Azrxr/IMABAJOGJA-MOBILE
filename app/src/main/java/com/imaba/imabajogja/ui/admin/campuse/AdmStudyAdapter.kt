...
import com.imaba.imabajogja.databinding.ItemStudyMembersBinding

class AdmStudyAdapter(
    private val onItemClick: (DataItemMember) -> Unit,
) : PagingDataAdapter<DataItemMember, AdmStudyAdapter.MemberViewHolder>(DIFF_CALLBACK) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MemberViewHolder {
        val binding =
            ItemStudyMembersBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    inner class MemberViewHolder(private val binding: ItemStudyMembersBinding) :
        ...
        fun bind(member: DataItemMember, onItemClick: (DataItemMember) -> Unit) {
            ...
            binding.rvStatusStudy.text = member.statusKuliah ?: context.getString(R.string.empty)
            ...
            binding.rvStatusBerkas.text = if (member.berkasLengkap == true) {
                "Lengkap"
            } else {
                "Belum Lengkap"
            }
            binding.rvProgressDoc.text = member.berkasProgress ?: context.getString(R.string.empty)
